...
parameters {
  ...
  real<lower=0,upper=30> tau_id_mu;
  real<lower=0,upper=30> tau_days_mu;
  // real<lower=0,upper=200> tau_batch_mu;
  real<lower=0,upper=30> tau_id_pi;
  real<lower=0,upper=30> tau_days_pi;
  // real<lower=0,upper=300> tau_batch_pi;
  
  // matrix[ncolY-1,ncolX] beta1; // coefficients (raw)
  vector[ncolY-1] A1; //parameters for the Genotype and Spline interactions
  
  // matrix[ncolY-1,ncolX] beta2; // coefficients (raw)
  // vector[ncolY-1] A2; //parameters for the Genotype and Spline interactions
  
  // matrix[ncolY-1,ncolX] beta3; // coefficients (raw)
  vector[ncolY-1] A3; //parameters for the Genotype and Spline interactions
...
transformed parameters{
  ...
  for (n in 1:N) { // linking the mean and dispersion to parameters & design matrix through logit transformation
    for (m in 1:(ncolY-1)){
      mu_logits[n,m] = X[n,] * transpose(params_mu[m,]) + beta_splines[n,m] + A1[m] * X[n,2] * beta_splines[n,m] + id[mice_id[n]] + days_id[days[n]];
      ...
      pi_logits[n,m] = X[n,] * transpose(params_pi[m,]) + beta_splines3[n,m] + A3[m] * X[n,2] * beta_splines3[n,m] + id3[mice_id[n]] + days_id3[days[n]];
...
generated quantities {
  int<lower=0,upper=3600> yrep[N, ncolY];
  for (n in 1:N) {
