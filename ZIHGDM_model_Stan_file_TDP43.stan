...
transformed parameters{
  ...
  for (n in 1:N) { // linking the mean and dispersion to parameters & design matrix through logit transformation
    for (m in 1:(ncolY-1)){
      mu_logits[n,m] = X[n,] * transpose(params_mu[m,]) + beta_splines[n,m] + A1[m] * X[n,3] * beta_splines[n,m] + id[mice_id[n]] + days_id[days[n]];
      ...
      pi_logits[n,m] = X[n,] * transpose(params_pi[m,]) + beta_splines3[n,m] + A3[m] * X[n,3] * beta_splines3[n,m] + id3[mice_id[n]] + days_id3[days[n]];
